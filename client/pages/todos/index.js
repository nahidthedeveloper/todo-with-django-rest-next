...
const index = () => {
    ...
    const todoStatusHandler = (id, status) => {
        ...
        httpClient.patch(`/todos/${id}`, {status})
            .then((response) => {
                setSubmit(!submit)
            }).catch(err => {
            console.log(err);
    ...
    return (
        <>
            ...
            {loading ? <p className='text-center mt-8'>
                    Loading...
                </p>
                :
                <div className="flex justify-center">
                    <div className="w-[600px] mt-8">
                        ...
                        <form className="space-y-4 md:space-y-6 mt-8" onSubmit={handleSubmit(addTodoHandler)}>
                            <div className="flex gap-4">
                                <div className='w-full'>
                                    ...
                                    />
                                    <p className={'text-red-400 pl-1 pt-2 text-sm'}>{errors.title?.message}</p>
                                </div>
                                <button
                        ...
                        {todos?.map(({id, title, status}) => (
                            <div key={id}>
                                <div className="flex justify-between items-center gap-3 mb-4">
                                    <p className={`${status === 'done' && 'line-through'} text-md overflow-auto break-all py-2`}>
                                        {title}
                                    </p>
                                    <div className="flex gap-4">
                                        <button
                                            ...
                                            onClick={() => todoStatusHandler(id, status)}
                                        >
                                            {status === 'done' ? "Done" : "In Progress"}
                                        </button>
                                        {status !== "done" && (
                                            <TitleUpdateModal id={id} title={title} submit={submit}
                                                              setSubmit={setSubmit}/>
                                        )}
                                        <button
